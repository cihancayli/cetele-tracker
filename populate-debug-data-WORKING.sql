-- ============================================
-- POPULATE DEBUG DATA FOR CETELE DASHBOARD (WORKING VERSION)
-- ============================================
-- Creates:
-- - 1 ED user
...
DO $$
BEGIN
    -- Try cetele_submissions first (for debug groups)
    IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'cetele_submissions') THEN
        EXECUTE 'DELETE FROM cetele_submissions WHERE student_id IN (SELECT s.id FROM students s JOIN groups g ON s.group_id = g.id WHERE g.name LIKE ''%Group'')';
    END IF;

    -- Try weekly_submissions (for debug groups)
    IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'weekly_submissions') THEN
        EXECUTE 'DELETE FROM weekly_submissions WHERE student_id IN (SELECT s.id FROM students s JOIN groups g ON s.group_id = g.id WHERE g.name LIKE ''%Group'')';
    END IF;
END $$;

-- Delete ALL students in debug groups (not just the ones with pattern)
DELETE FROM students WHERE group_id IN (SELECT id FROM groups WHERE name LIKE '%Group');
...
-- Mentor 1 - Aziz
INSERT INTO users (id, username, email, password_hash, role, region, is_coordinator, is_mentor, created_at, last_login)
VALUES ('10000000-0000-0000-0000-000000000011', 'aziz_mentor', 'aziz@debug.cetele', 'ZGVidWcxMjM=', 'mentor', 'High School', false, true, NOW(), NOW())
ON CONFLICT (email) DO UPDATE SET role = EXCLUDED.role, is_coordinator = EXCLUDED.is_coordinator, is_mentor = EXCLUDED.is_mentor, last_login = NOW();

-- Mentor 2 - Cihan
INSERT INTO users (id, username, email, password_hash, role, region, is_coordinator, is_mentor, created_at, last_login)
VALUES ('10000000-0000-0000-0000-000000000012', 'cihan_mentor', 'cihan@debug.cetele', 'ZGVidWcxMjM=', 'mentor', 'High School', false, true, NOW(), NOW())
ON CONFLICT (email) DO UPDATE SET role = EXCLUDED.role, is_coordinator = EXCLUDED.is_coordinator, is_mentor = EXCLUDED.is_mentor, last_login = NOW();

-- Mentor 3 - Fatima
INSERT INTO users (id, username, email, password_hash, role, region, is_coordinator, is_mentor, created_at, last_login)
VALUES ('10000000-0000-0000-0000-000000000013', 'fatima_mentor', 'fatima@debug.cetele', 'ZGVidWcxMjM=', 'mentor', 'High School', false, true, NOW(), NOW())
ON CONFLICT (email) DO UPDATE SET role = EXCLUDED.role, is_coordinator = EXCLUDED.is_coordinator, is_mentor = EXCLUDED.is_mentor, last_login = NOW();

-- Mentor 4 - Omar
INSERT INTO users (id, username, email, password_hash, role, region, is_coordinator, is_mentor, created_at, last_login)
VALUES ('10000000-0000-0000-0000-000000000014', 'omar_mentor', 'omar@debug.cetele', 'ZGVidWcxMjM=', 'mentor', 'High School', false, true, NOW(), NOW())
ON CONFLICT (email) DO UPDATE SET role = EXCLUDED.role, is_coordinator = EXCLUDED.is_coordinator, is_mentor = EXCLUDED.is_mentor, last_login = NOW();

-- Mentor 5 - Aisha
INSERT INTO users (id, username, email, password_hash, role, region, is_coordinator, is_mentor, created_at, last_login)
VALUES ('10000000-0000-0000-0000-000000000015', 'aisha_mentor', 'aisha@debug.cetele', 'ZGVidWcxMjM=', 'mentor', 'High School', false, true, NOW(), NOW())
ON CONFLICT (email) DO UPDATE SET role = EXCLUDED.role, is_coordinator = EXCLUDED.is_coordinator, is_mentor = EXCLUDED.is_mentor, last_login = NOW();

-- Mentor 6 - Yusuf
INSERT INTO users (id, username, email, password_hash, role, region, is_coordinator, is_mentor, created_at, last_login)
VALUES ('10000000-0000-0000-0000-000000000016', 'yusuf_mentor', 'yusuf@debug.cetele', 'ZGVidWcxMjM=', 'mentor', 'High School', false, true, NOW(), NOW())
ON CONFLICT (email) DO UPDATE SET role = EXCLUDED.role, is_coordinator = EXCLUDED.is_coordinator, is_mentor = EXCLUDED.is_mentor, last_login = NOW();
...
BEGIN
    ...
    IF activity_count < 10 THEN
        ...
        INSERT INTO activities (id, name, description, type, order_index, created_at) VALUES
            ('40000000-0000-0000-0000-000000000001', 'Kuran', 'Quran recitation', 'spiritual', 1, NOW()),
            ('40000000-0000-0000-0000-000000000002', 'Kitap', 'Book reading', 'education', 2, NOW()),
            ('40000000-0000-0000-0000-000000000003', 'Teheccud', 'Night prayer', 'spiritual', 3, NOW()),
            ('40000000-0000-0000-0000-000000000004', 'Salat', 'Daily prayers', 'spiritual', 4, NOW()),
            ('40000000-0000-0000-0000-000000000005', 'Zikr', 'Remembrance', 'spiritual', 5, NOW()),
            ('40000000-0000-0000-0000-000000000006', 'Dua', 'Supplication', 'spiritual', 6, NOW()),
            ('40000000-0000-0000-0000-000000000007', 'Sadaka', 'Charity', 'social', 7, NOW()),
            ('40000000-0000-0000-0000-000000000008', 'Halaqa', 'Study circle', 'education', 8, NOW()),
            ('40000000-0000-0000-0000-000000000009', 'Volunteer', 'Community service', 'social', 9, NOW()),
            ('40000000-0000-0000-0000-000000000010', 'Hadith', 'Hadith study', 'education', 10, NOW());
    END IF;
END $$;
...
BEGIN
    ...
    ELSIF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'weekly_submissions') THEN
    ...
    FOR week_offset IN 0..2 LOOP
        ...
        FOR student_record IN SELECT id FROM students WHERE name ~ '^[A-Z][a-z]+ [a-z0-9]{4}$' LOOP
            ...
            IF table_name = 'cetele_submissions' THEN
                EXECUTE 'INSERT INTO cetele_submissions (id, student_id, week_start_date, activity_completions, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT (student_id, week_start_date) DO UPDATE SET activity_completions = EXCLUDED.activity_completions, updated_at = EXCLUDED.updated_at'
                USING uuid_generate_v4(), student_record.id, week_start, completions, NOW(), NOW();
            ELSE
                EXECUTE 'INSERT INTO weekly_submissions (id, student_id, week_start_date, activity_completions, created_at) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (student_id, week_start_date) DO UPDATE SET activity_completions = EXCLUDED.activity_completions'
                USING uuid_generate_v4(), student_record.id, week_start, completions, NOW();
            END IF;
        END LOOP;
    END LOOP;
...
BEGIN
    ...
    IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'cetele_submissions') THEN
        table_name := 'cetele_submissions';
        EXECUTE 'SELECT COUNT(*) FROM cetele_submissions' INTO submission_count;
    ELSIF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'weekly_submissions') THEN
        table_name := 'weekly_submissions';
        EXECUTE 'SELECT COUNT(*) FROM weekly_submissions' INTO submission_count;
    ELSE
        table_name := 'NONE';
        submission_count := 0;
    END IF;
