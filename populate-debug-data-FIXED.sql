-- ============================================
-- POPULATE DEBUG DATA FOR CETELE DASHBOARD (FIXED)
-- ============================================
-- Creates:
-- - 1 ED user
...
UNION ALL SELECT 'GROUPS', COUNT(*) FROM groups WHERE name LIKE '%Group'
UNION ALL SELECT 'STUDENTS', COUNT(*) FROM students WHERE name ~ '^[A-Z][a-z]+ [a-z0-9]{4}$'
UNION ALL SELECT 'ACTIVITIES', COUNT(*) FROM activities WHERE is_active = true
UNION ALL SELECT 'MENTOR CODES', COUNT(*) FROM mentor_codes WHERE code LIKE 'MENTOR-%'
UNION ALL SELECT 'SUBMISSIONS (cetele)', COALESCE((SELECT COUNT(*) FROM cetele_submissions WHERE 1=1), 0)
UNION ALL SELECT 'SUBMISSIONS (weekly)', COALESCE((SELECT COUNT(*) FROM weekly_submissions WHERE 1=1), 0)
ORDER BY type;
